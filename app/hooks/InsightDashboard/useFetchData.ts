...
const useFetchData = () => {
  const { addReports, reports, userId, attributes, addAttributes, projects, addProjects } = useDashboardStore();
  const [attributesFetched, setAttributesFetched] = useState(false);

  const fetchAttributes = useCallback(async () => {
    try {
      console.log('Fetching attributes', userId);
      if (!userId) {
  ...
  return { fetchAttributes, fetchProjects, fetchAllData };
};
